...
def can_earn_today(user_id):
    ...
    if last_activity < today:
        ...
        # Clean old dates
        completed_tasks[user_id] = {date: tasks for date, tasks in completed_tasks[user_id].items() if date == today_str}
...
def has_completed_task_today(user_id, task_key):
    ...
    if user_id not in completed_tasks:
        completed_tasks[user_id] = {}
    if today not in completed_tasks[user_id]:
        completed_tasks[user_id][today] = set()
    return task_key in completed_tasks[user_id][today]
...
def mark_task_completed(user_id, task_key):
    ...
    if today not in completed_tasks[user_id]:
        completed_tasks[user_id][today] = set()
    completed_tasks[user_id][today].add(task_key)
...
""")
    ...
    keyboard = [
        [InlineKeyboardButton(f"{safe_emoji('thumbs_up', '👍')} Like Video", callback_data="task_like"),
         InlineKeyboardButton(f"{safe_emoji('comment', '💬')} Comment", callback_data="task_comment")],
        [InlineKeyboardButton(f"{safe_emoji('bell', '🔔')} Subscribe", callback_data="task_subscribe"),
         InlineKeyboardButton(f"{safe_emoji('eyes', '👀')} Watch 45s", callback_data="task_watch")],
        [InlineKeyboardButton(f"{safe_emoji('video', '📺')} Watch 3min", callback_data="task_watch_3min"),
         InlineKeyboardButton(f"{safe_emoji('news', '📰')} Visit Article", callback_data="task_visit")],
        [InlineKeyboardButton(f"{safe_emoji('back', '🔙')} Back to Menu", callback_data="start_menu")]
    ]
...
""")
    ...
    # Get task buttons
    task_buttons = get_task_buttons(task_key)
